...
%define stablelibre -5.13%{?stablegnux}
#define rcrevlibre  -5.13%{?rcrevgnux}
#define gitrevlibre -5.13%{?gitrevgnux}
...
Requires: kernel-libre-core-uname-r = %{KVERREL}
Requires: kernel-libre-modules-uname-r = %{KVERREL}
%endif
...
Provides: kernel-libre%{?1:-%{1}}-modules-internal-%{_target_cpu} = %{version}-%{release}\
Provides: kernel%{?1:-%{1}}-modules-internal-%{_target_cpu} = %{version}-%{release}%{?1:+%{1}}\
Provides: kernel-libre%{?1:-%{1}}-modules-internal-%{_target_cpu} = %{version}-%{release}%{?1:+%{1}}\
Provides: kernel%{?1:-%{1}}-modules-internal = %{version}-%{release}%{?1:+%{1}}\
Provides: kernel-libre%{?1:-%{1}}-modules-internal = %{version}-%{release}%{?1:+%{1}}\
Provides: installonlypkg(kernel-module)\
Provides: installonlypkg(kernel-libre-module)\
Provides: kernel%{?1:-%{1}}-modules-internal-uname-r = %{KVERREL}%{?1:+%{1}}\
Provides: kernel-libre%{?1:-%{1}}-modules-internal-uname-r = %{KVERREL}%{?1:+%{1}}\
...
Requires: kernel-libre%{?1:-%{1}}-modules-uname-r = %{KVERREL}%{?1:+%{1}}\
%if %{-m:1}%{!-m:0}\
Requires: kernel-libre-modules-extra-uname-r = %{KVERREL}\
%endif\
AutoReq: no\
AutoProv: yes\
...
Requires: kernel-libre-%{1}-core-uname-r = %{KVERREL}+%{1}\
Requires: kernel-libre-%{1}-modules-uname-r = %{KVERREL}+%{1}\
Provides: installonlypkg(kernel-libre)\
Provides: installonlypkg(kernel)\
%description %{1}\
...
ApplyPatch()
{
  ...
  if ! grep -E "^Patch[0-9]+: $patch\$" %{_specdir}/${RPM_PACKAGE_NAME%%%%-libre}.spec ; then
...
InitBuildVars() {
    ...
    KernelVer=%{version}-libre.%{release}.%{_target_cpu}${Variant:++${Variant}}

    # make sure EXTRAVERSION says what we want it to say
    # Trim the release if this is a CI build, since KERNELVERSION is limited to 64 characters
    ShortRel=$(perl -e "print \"%{release}\" =~ s/\.pr\.[0-9A-Fa-f]{32}//r")
    perl -p -i -e "s/^EXTRAVERSION.*/EXTRAVERSION = -libre.${ShortRel}.%{_target_cpu}${Variant:++${Variant}}/" Makefile
...
* Sat Sep 18 2021 Justin M. Forbes <jforbes@fedoraproject.org> [5.13.19-200]
...
- GNU Linux-libre 5.13.16-gnu1.

* Mon Sep 13 2021 Justin M. Forbes <jforbes@fedoraproject.org> [5.13.16-200]
- kernel-5.13.16-0 (Justin M. Forbes)

* Thu Sep  9 2021 Alexandre Oliva <lxoliva@fsfla.org> -libre
- GNU Linux-libre 5.13.15-gnu1.

* Wed Sep 08 2021 Justin M. Forbes <jforbes@fedoraproject.org> [5.13.15-200]
...
* Thu Aug 19 2021 Alexandre Oliva <lxoliva@fsfla.org> -libre
- GNU Linux-libre 5.13.12-gnu.
