/* PainelImprimirDARFCodigoBarras - Decompiled by JODE
...
public class PainelImprimirDARFCodigoBarras extends JPanel
{
  ...
  {
    ((JDialog) SwingUtilities.getRoot (this)).setVisible (false);
    final int[] rows = edtTableDecs.getSelectedRows ();
    if (rows.length > 0)
	...
	public Object run ()
	{
	  ...
	  if (dec.getResumo ().getCalculoImposto ().getValorQuota ().comparacao (">=", minimoQuota))
	    {
	      boolean[] quotas = new boolean[qtdeQuota];
	      quotas[0] = true;
	      for (int i = 1; i < qtdeQuota; i++)
		quotas[i] = false;
	      ImpressaoDarf imp = new ImpressaoDarf ("Darf", "relDarfCodBarra.jasper", pathDados, "/classe", cpf, quotas, nome, dados);
	      imp.vizualizarDarfCodBarra ();
	    }
	  else
